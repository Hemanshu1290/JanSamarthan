...
function matchNGOs(requirement, ngos) {
  // HARD FILTER: only NGOs that actually work in the requested sector are
  // considered at all. Without this, every NGO showed up for every sector
  // search, just re-ordered by trust score.
  const inSector = requirement.category
    ? ngos.filter((ngo) => ngo.category.includes(requirement.category))
    : ngos;

  const results = inSector.map((ngo) => {
